...
if [ $? != 0 ]; then
  ...
  tmux split-window  -t "$SESH":"$window".1 -h
  tmux send-keys     -t "$SESH":"$window".1 "cd ~/work/idx; gsll" C-m
  tmux send-keys     -t "$SESH":"$window".2 "cd ~/work/idx; hx daily-log.md notes/personal-meeting-notes.md" C-m
  tmux send-keys     -t "$SESH":"$window".3 "cd ~/work/idx/notes; hx useful_commands.md reference.md" C-m
  tmux select-layout -t "$SESH":"$window" even-horizontal
